...
export const fetchAllProduct = (category, name) => {
  if (category && name) {
    ...
    return Axios.get(
      `${API_URL}/product?name=&brand=&category=&page=1&req=DESC&limit=15`
    );
  }
};
...
export const sendEmailSeller = (data) => {
  return Axios.post(`${API_URL}/auth/sendemailseller`, data);
};
...
export const resetPasswordSeller = (data) => {
  return Axios.post(`${API_URL}/auth/resetpassseller`, data);
};
